...
| v23 **모프/마스크** 레코드 | `version >= 23` | 디컴파일 `:345 if (0x16 < iVar11)` |
...
### 1.7 인덱스 폭 = **메시별 플래그** `2 + 2*(gateWord & 1)`

> ~~### 1.7 인덱스 폭 = **u16** (실물 반증으로 확정)~~
>
> **[2026-08-30 정정 — 이 절은 코퍼스 상관을 엔진 규칙으로 오인했다.]**
> 제목의 "확정" 이 틀렸다. 2026-08-28 커밋 `0bb963ed`(*"인덱스 폭은 u16 고정이 아니라
> 메시별 플래그다"*)가 `corpus_scan/mdl-format.md:163-184` 를 이 방향으로 뒤집었지만
> **이 리포트의 §1.7 과 §4-5 는 그 스윕에서 빠졌다.** 그 커밋이 이 파일의 §3(TEXS) 훅은
> 고쳤으므로, 이 절이 남은 것은 동결된 과거 스냅샷이 아니라 누락이다.
>
> 엔진은 메시 구조체 `[mesh + 0x18]` 게이트워드의 **비트 0** 으로 원소 폭을 계산한다.
> 무결 원본 `wallpaper_engine/wallpaper64.exe` 에서 다시 읽은 바이트:
>
> ```
> 0x1401d784c  0f b6 4f 18            movzx ecx, byte [rdi+0x18]   ; gateWord
> 0x1401d7853  80 e1 01               and   cl, 1
> 0x1401d7870  46 8d 0c 55 02 00 00 00 lea  r9d, [r10*2+2]         ; 폭 = 2 + 2*bit0
> 0x1401d7878  41 f7 f9               idiv  r9d
> 0x14009a98d  85 d2                  test  edx, edx               ; 소비처
...
> ```
>
> 그 함수 `0x1401d7760` 은 이 저장소의 재생성 코퍼스에 있다
> (`analysis/decompiled/all/00000001401d7760__FUN_1401d7760.c`).
>
> **실물에 u32 메시가 있다.** Waple 정본 `spec/formats/mdl-deep.json` →
> `format.mdl.indexWidth`(status 확정)가 `.mdl` 451개 전수에서
> **u16 969메시 / u32 17메시**(u32 최소 정점수 **69,396**)를 기록하고, 플래그 규칙으로
> 읽으면 `maxIndex == vertexCount-1` 이 986/986 이다.
> `Sources/WapleCore/Model3D.swift:752` 가 `let iWidth = (gateWord & 1) == 0 ? 2 : 4` 다.
>
> **오판 시 조용히 깨진다.** u32 블롭을 u16 로 읽으면 상위 워드 0 이 섞여 maxIndex 가
> 정확히 `0xFFFF` 로 찍히고(실측 17메시 전부), 그 값은 해당 메시 정점 수보다 작으므로
> `maxIndex < vertexCount` 류의 가드로는 절대 안 걸린다.
>
> **아래 측정 자체는 전부 참이다 — 다만 "이 28파일이 u16 을 쓴다" 는 코퍼스 사실이지
> 엔진 규칙이 아니다.** 설치본 45메시가 전건 `gateWord == 0` 이고 최대 정점수 10,995 라
> 두 규칙이 같은 답을 낸다. **아래 상관으로부터 재구현하지 마라.**

디코더 함수 자체는 인덱스 블롭을 원소 단위로 해석하지 않고 포인터+길이로 메시 구조체에
넘긴다. 폭을 정하는 코드는 디코더 밖 GPU 업로드 경로(`0x1401d7760`, 파스 직후
`0x1401d5bb1` 에서 호출)에 있다. 아래는 이 저장소 실물 28파일의 **코퍼스 상관**이다:
45메시 전부 `indexBlobLen % 6 == 0`(u16 삼각형)이고, **5메시는 `indexBlobLen % 12 != 0`**
이라 u32 삼각형이 **그 5메시에 대해서는** 불가능하다.
...
- `MDLV0023`: 끝에 `00 00 00 00` + `0x00` = v23 **모프/마스크** 카운트 0 + 빈 태그.
...
Waple 저장소(`<WAPLE>`, **읽기만 함**)의 MDL 파서는
...
| ~~기타~~ **TEXS** | `0x14015e1d0` | ~~프레임/시퀀스 추정~~ **스프라이트시트/프레임 테이블** |

> **[2026-08-30 정정 — 이 표의 3행이 §3 의 2026-08-28 정정을 따라오지 않았다.]**
> `0x14015e1d0` 은 "기타(catch-all)" 가 아니라 **`TEXS0003` 전용 파서**다. TEXI/TEXB/TEXS
> 어느 것도 아닌 태그에는 핸들러가 **없다** — `0x14015e7f9` 의 `jnz` 로 `0x14015e86b` 에 가서
> 루프를 빠져나간다. 역할도 추정이 아니다: `i32 frameCount` 뒤 프레임 지오메트리,
> `v>=3` 이면 gif 폭·높이 명시(레이아웃 전문은 Waple `docs/re/tex-format.md` §1.3).
> 이 저장소 `.tex` 440개 중 **61개**가 TEXS 섹션을 갖는다
> (`grep -c` 상당: `TEXS000` 바이트열 보유 61/440). 상세는 위 §3 의 2026-08-28 정정 블록.
...
3. ~~**v23 본 바인딩 레코드의 뒷부분.**~~ ✅ **해소(정정 2026-09-01) — 본 바인딩이 아니라
   모프/마스크 레코드다.** 프레이밍 `{u64; cstring; u32; u32 k → k×u32; u32 m → m×u32}` 은 그대로
   맞고 28파일이 정확히 착지하는 것도 그대로지만, **이 리포의 28파일은 여전히 카운트가 전부 0**
   이라 여기서는 프레이밍만 검증된다. 본문을 실제로 태우는 것은 Waple 쪽의 더 넓은 워크샵
   코퍼스 12파일이고, 거기서 관측된 이름은 전부 `masks/clipping_mask_*` 다 — 스키닝이 아니라
   클리핑 마스크 계열이다. Waple 은 이것을 `Model3D.MorphTarget` 으로 파스·보존한다(렌더 소비는
   범위 밖). 정본 서술은 `corpus_scan/mdl-format.md` §「v23 morph/mask record」 다.
   **이름을 '본 바인딩' 으로 부르면 스키닝 구현자가 이 레코드를 찾아 헤맨다 — 그래서 용어를 바꿨다.**
...
5. ~~**인덱스 폭을 엔진이 어디서 정하는가.** u16 이라는 결론은 실물 산술 반증(§1.7)이고,
   ...
   **[2026-08-30 해결 — 미해결 목록에서 내린다.]** 폭을 정하는 코드는 찾았다. 디코더 밖
   `.mdl` 전용 GPU 업로드 경로 `0x1401d7760`(파스 직후 `0x1401d5bb1` 에서 호출)이
   `movzx ecx,byte[rdi+0x18]`(`0x1401d784c`) → `and cl,1`(`0x1401d7853`) →
   `lea r9d,[r10*2+2]`(`0x1401d7870`) → `idiv r9d`(`0x1401d7878`) 로 **메시별로**
   `2 + 2*(gateWord & 1)` 을 고른다. 그 플래그가 인자로 넘어가 소비처 `0x14009a98d` 에서
   `test edx,edx` → `cmove` 로 `0x39`(R16_UINT)/`0x2a`(R32_UINT) 를 갈라 잡는다 —
   즉 `IASetIndexBuffer` 인자가 여기서 결정되므로 별도 확인이 필요 없다. 상세는 §1.7 정정과
   Waple 정본 `spec/formats/mdl-deep.json` → `format.mdl.indexWidth`(status 확정).
...
8. ~~**`0x14015e1d0`(TEXI/TEXB/TEXS 가 아닌 섹션 처리기)의 역할.** "프레임/시퀀스"는 추정이다.~~
   **[2026-08-30 해결 — 전제부터 틀렸다.]** `0x14015e1d0` 은 "TEXI/TEXB/TEXS 가 아닌"
   처리기가 아니라 **`TEXS0003` 전용 파서**다(§3 의 2026-08-28 정정). 역할도 추정이 아니라
   **스프라이트시트/프레임 테이블**로 확인됐다 — `i32 frameCount`(`0x14015e21c`) 뒤 프레임
   지오메트리, `v>=3` 이면 gif 폭·높이 명시. 레이아웃 전문은 Waple
   `docs/re/tex-format.md` §1.3, 구현은 `Sources/WapleCore/TexImage.swift`.
   TEXI/TEXB/TEXS 아무것도 아닌 태그에는 핸들러가 없다(루프를 빠져나간다).
...
`vlen % stride == 0` + `ilen % 6 == 0`(실패 0), `ilen % 12 != 0` 인 메시 5개
(**이 저장소 28파일에 한정된 코퍼스 사실** — 엔진 규칙은 메시별 `2 + 2*(gateWord & 1)` 이다,
§1.7 정정 참조),
